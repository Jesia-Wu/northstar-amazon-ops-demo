...
describe('Sites 静态 Worker 入口', () => {
  it('把客户端静态文件与 Worker 分别输出到 Sites 约定目录', () => {
    const prepareScript = readFileSync(join(process.cwd(), 'scripts', 'prepare-sites-worker.mjs'), 'utf8');

    expect(prepareScript).toContain("'client'");
    expect(prepareScript).toContain('rename(');
