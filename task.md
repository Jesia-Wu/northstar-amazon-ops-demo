...
- 当前状态：本地桌面端 React 演示站已完成并通过验证；正在按私有静态发布包生成可直接打开的试用地址。
- 下一动作：完成静态发布构建、私有托管与部署状态核验；若平台不能保证私有访问则停止，不发布为公开站点。
...
- 当前任务：做一版实际可运行的本地 Amazon 运营控制台初级网站，使用内置演示数据，不接 MCP、Amazon、LLM 或任何外部数据服务。
